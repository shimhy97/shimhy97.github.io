...
def _summarize_with_openai(
    ...
    system_prompt = (
        "너는 한국어 기술 블로그 에디터다. "
        "공식 소스 기반 최신 기술 소식을 사실에 근거해 하나의 흐름으로 묶는다. "
        "핵심은 항목별 요약이 아니라 변화의 맥락을 재구성하는 것이다. "
        "과장, 홍보성 표현, 확인되지 않은 추측을 금지한다. "
        "반드시 JSON만 반환하라."
    )
    user_prompt = (
        ...
        "overview는 이번 주 글의 중심 관찰이 드러나는 2~3문장으로 쓰고, "
        "items의 각 필드는 블로그 초안 문단으로 재구성하기 쉬운 재료가 되게 써라. "
        "구체 명사, 기능명, 버전, 날짜, 시점 같은 확인 가능한 단서를 우선하고, "
        "같은 표현과 추상 문장을 반복하지 마라. "
