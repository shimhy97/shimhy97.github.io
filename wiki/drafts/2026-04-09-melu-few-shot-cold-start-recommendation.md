...
MeLU는 콜드스타트 추천 논문으로 자주 소개되지만, 실제로 읽어 보면 핵심은 "처음부터 다 아는 추천기"보다 "몇 개만 보고도 빨리 적응하는 추천기"에 가깝다. 그래서 이 글은 성능 표를 나열하기보다, 새 유저에게 실제로 어떤 순서로 추천이 이뤄지는지와 `support set`, `query set`, `local update`가 서비스 흐름에서 무엇을 뜻하는지 다시 번역하는 데 집중한다.
...
- [MeLU는 새 유저에게 어떻게 추천하고, local update는 누구 파라미터를 바꾸나](../queries/2026-04-09-how-melu-serves-a-new-user.md)
